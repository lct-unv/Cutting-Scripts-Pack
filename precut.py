...
for ma in range(2,5):
    ...
    av.append(am1)
f1=open("./wannier90_hr.dat_primed_nsymm_6","r")
data=[]
for d in f1.readlines():
    d1=[float(p) for p in d.strip().split()]
    data.append(d1)
f1.close()
long=float(input("Enter the length of the cut-triangle: "))
shift1=float(input("Enter the left-shift-x of the cut-triangle: "))
shift2=float(input("Enter the down-shift-y of the cut-triangle: "))
...
f2=open("./wannier90_hr.dat_primed_nsymm_6S","w")
f2.writelines(datao)
f2.close()
